...
{	
	// Modified by TSP - to support multiple database connections make the cache multidemensional
	// based on database name then table
	static $table_fields_cache = array();
	
	if (!isset($table_fields_cache[$dbc_name][$table_name])) {
		$table_fields_cache[$dbc_name][$table_name] = db_get_fields("$dbc_name#SHOW COLUMNS FROM ?:$table_name");
	}
	
	$fields = $table_fields_cache[$dbc_name][$table_name];
	if (!$fields) {
		return false;
	}
